"""IoT Gateway – API Key authentication demo (Python client)."""
...
VALID_API_KEY = "key-abc-123"
INVALID_API_KEY = "key-WRONG-000"
...
def send_telemetry(api_key, label):
    separator(label)
    body = {
        "temperature": round(20 + random.random() * 10, 1),
        "humidity": round(40 + random.random() * 40),
    }
    print(f"{GREEN}{BOLD}[GATEWAY]{RESET} POST /api/telemetry  X-API-Key: {api_key}")
...
if __name__ == "__main__":
    send_telemetry(VALID_API_KEY, "Step 1: Send telemetry with VALID API key")
    list_telemetry()
    send_telemetry(INVALID_API_KEY, "Step 3: Send telemetry with INVALID API key")
